...
**No dependency analysis needed.** Wave numbers are pre-computed during `/prompts:gsd-plan-phase`.
...
Resume relies on Codex CLI's internal serialization which breaks with parallel tool calls.
...
| `gaps_found` | Present gap summary, offer `/prompts:gsd-plan-phase {phase} --gaps` |
...
- `/prompts:gsd-verify-work {X}` — manual testing before planning
```

User runs `/prompts:gsd-plan-phase {X} --gaps` which:
1. Reads VERIFICATION.md gaps
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close gaps
3. User then runs `/prompts:gsd-execute-phase {X} --gaps-only`
...
1. Run `/prompts:gsd-execute-phase {phase}` again
