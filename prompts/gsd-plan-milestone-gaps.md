...
<objective>
Create all phases necessary to close gaps identified by `/prompts:gsd-audit-milestone`.

Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase.

One command creates all fix phases — no manual `/prompts:gsd-add-phase` per gap.
</objective>
...
```
No audit gaps found. Run `/prompts:gsd-audit-milestone` first.
```
...
**Also available:**
- `/prompts:gsd-execute-phase {N}` — if plans already exist
...
`/prompts:gsd-audit-milestone` — re-audit to verify gaps closed
`/prompts:gsd-complete-milestone {version}` — archive when audit passes
```
...
- [ ] Changes committed
- [ ] User knows to run `/prompts:gsd-plan-phase` next
</success_criteria>
