...
Example: `/prompts:gsd-insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /prompts:gsd-insert-phase <after> <description>"
  echo "Example: /prompts:gsd-insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /prompts:gsd-plan-phase {decimal_phase} to break down)
...
`/prompts:gsd-plan-phase {decimal_phase}`
...
- Don't use this for planned work at end of milestone (use /prompts:gsd-add-phase)
