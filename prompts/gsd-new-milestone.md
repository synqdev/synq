...
**After this command:** Run `/prompts:gsd-plan-phase [N]` to start execution.
...
1. **Verify project exists:**
   ```bash
   [ -f .planning/PROJECT.md ] || { echo "ERROR: No PROJECT.md. Run /prompts:gsd-new-project first."; exit 1; }
   ```
...
2. **Check for active milestone (ROADMAP.md exists):**
   ...
   **If ACTIVE_MILESTONE:**
   ...
   - options:
     - "Complete current first" — Run /prompts:gsd-complete-milestone
     - "Continue anyway" — Start new milestone (will archive current)

   If "Complete current first": Exit with routing to `/prompts:gsd-complete-milestone`
...
`/prompts:gsd-discuss-phase [N]` — gather context and clarify approach
...
**Also available:**
- `/prompts:gsd-plan-phase [N]` — skip discussion, plan directly
...
- [ ] User knows next step is `/prompts:gsd-plan-phase [N]`
