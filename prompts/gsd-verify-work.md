...
Output: {phase}-UAT.md tracking all test results. If issues found: diagnosed gaps, verified fix plans ready for /prompts:gsd-execute-phase
</objective>
...
8. If issues found:
   ...
   - Present ready status with `/clear` then `/prompts:gsd-execute-phase`
</process>
...
- /prompts:gsd-execute-phase {Z+1} — skip to execution (if already planned)
...
/prompts:gsd-execute-phase {Z} --gaps-only
...
- /prompts:gsd-plan-phase {Z} --gaps — regenerate fix plans
...
**Options:**
- /prompts:gsd-plan-phase {Z} --gaps — retry fix planning with guidance
- /prompts:gsd-discuss-phase {Z} — gather more context before replanning
...
- [ ] Ready for `/prompts:gsd-execute-phase` when complete
</success_criteria>
